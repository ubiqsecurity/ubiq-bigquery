...
class FF1 extends FFX {
	...
	cipher(X, twk, encrypt) {
		...
		arrayCopy(twk, 0, PQ, p, twk.length); // arrayUtil
		...
		for (let i = 0; i < 10; i++) {
			...
			*/
			c = bigint_set_str(B, this.custom_radix_str, this.radix); // Bn
			...
			if (numb[0] === 0 && numb.length > 1) {
				 ...
				 */
				numb = copyOfRange(numb, 1, numb.length); // arrayUtil
			}
			if (b <= numb.length) {
				arrayCopy(numb, 0, PQ, PQ.length - b, b); // arrayUtil
			} else {
				/* pad on the left with zeros */
				PQ.fill(0, PQ.length - b, PQ.length - numb.length);
				arrayCopy(numb, 0, PQ, PQ.length - numb.length, numb.length); // arrayUtil
			}
			/* Step 6ii */
			...
			c = bigint_set_str(A, this.custom_radix_str, this.radix); // Bn
